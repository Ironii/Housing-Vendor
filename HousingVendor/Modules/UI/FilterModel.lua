...
function FilterModel:GetDefaultShowOnlyAvailable()
    if _G.HousingVersionFilter and _G.HousingVersionFilter.IsBetaClient then
        local ok, isBeta = pcall(_G.HousingVersionFilter.IsBetaClient, _G.HousingVersionFilter)
        if ok and isBeta then
            -- Beta/PTR: show Midnight/unreleased so testing is possible without extra steps.
            return false
        end
    end
    return true
