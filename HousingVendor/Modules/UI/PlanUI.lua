...
local function AddPlanTooltipLines(row, unitPrice)
    ...
    local function TitleCaseSource(src)
        ...
        if src == "gather" then return "Gather" end
        if src == "craft" then return "Craft" end
        return "Unknown"
    end
    ...
    _G.GameTooltip:AddLine("Crafting List", 1, 1, 1)
    if row and row.source then
        _G.GameTooltip:AddDoubleLine("Source", TitleCaseSource(row.source), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
    end
    _G.GameTooltip:AddDoubleLine("Required", tostring(required), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
    _G.GameTooltip:AddDoubleLine("Owned (bags)", tostring(ownedBags), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
    _G.GameTooltip:AddDoubleLine("Owned (bank)", tostring(ownedBank), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
    _G.GameTooltip:AddDoubleLine("Owned (warband)", tostring(ownedWarband), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
    _G.GameTooltip:AddDoubleLine("Owned (alts)", tostring(ownedAlts), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
    _G.GameTooltip:AddDoubleLine("Owned (total)", tostring(ownedTotal), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
    _G.GameTooltip:AddDoubleLine("Missing", tostring(missing), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
    if missingWithAlts ~= missing then
        _G.GameTooltip:AddDoubleLine("Missing (with alts)", tostring(missingWithAlts), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
    end

    unitPrice = tonumber(unitPrice)
    if unitPrice and unitPrice > 0 then
        _G.GameTooltip:AddLine(" ")
        _G.GameTooltip:AddDoubleLine("AH unit", FormatMoneyFromCopper(unitPrice), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
        _G.GameTooltip:AddDoubleLine("AH missing", FormatMoneyFromCopper(missing * unitPrice), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
        _G.GameTooltip:AddDoubleLine("AH all", FormatMoneyFromCopper(required * unitPrice), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9)
    end
    ...
    if type(usedBy) == "table" and next(usedBy) ~= nil then
        ...
        _G.GameTooltip:AddLine(" ")
        _G.GameTooltip:AddLine("Used by:", 1, 1, 1)
        local maxLines = 8
        ...
        if #entries > maxLines then
            _G.GameTooltip:AddLine("...and " .. tostring(#entries - maxLines) .. " more", 0.7, 0.7, 0.7)
        end
    end
...
local function GetSelectedItemSummary(itemID)
    local id = tonumber(itemID)
    if not id then return "Select an item in the list to see details." end
...
local function AddRecipeKnownToTooltip(known)
    if not (_G.GameTooltip and _G.GameTooltip.AddLine) then
        return
    end
    if known == true then
        _G.GameTooltip:AddLine("Recipe: Known", 0.95, 0.85, 0.25)
        _G.GameTooltip:Show()
    elseif known == false then
        _G.GameTooltip:AddLine("Recipe: Unknown", 0.9, 0.5, 0.5)
        _G.GameTooltip:Show()
    end
end
...
function PlanUI:CreateFrame()
    ...
    planFrame = CreateFrame("Frame", "HousingPlanFrame", UIParent, "BackdropTemplate")
    planFrame:SetSize(760, 520)
    planFrame:SetPoint("CENTER")
    planFrame:SetFrameStrata("DIALOG")
    planFrame:Hide()
    table.insert(UISpecialFrames, "HousingPlanFrame")
    ...
    planFrame:SetBackdropColor(bg[1], bg[2], bg[3], bg[4])
    planFrame:SetBackdropBorderColor(border[1], border[2], border[3], border[4])

    planFrame:SetMovable(true)
    planFrame:EnableMouse(true)
    planFrame:RegisterForDrag("LeftButton")
    planFrame:SetClampedToScreen(true)
    planFrame:SetScript("OnDragStart", function(self) self:StartMoving() end)
    planFrame:SetScript("OnDragStop", function(self) self:StopMovingOrSizing() end)

    local title = planFrame:CreateFontString(nil, "ARTWORK", "GameFontNormalLarge")
    title:SetPoint("TOPLEFT", 14, -12)
    title:SetText("Crafting List")
    ...
    summary:SetText("Targets: 0  |  Mats (req/miss): 0/0  |  Est. cost (missing): 0")
    ...
    helpBtn:SetScript("OnEnter", function(selfBtn)
        ...
        _G.GameTooltip:AddLine("Crafting List", 1, 1, 1)
        _G.GameTooltip:AddLine("Add decor you plan to craft to see combined reagents.", 0.85, 0.85, 0.85, true)
        _G.GameTooltip:AddLine("Left: crafting targets. Click a row, use X to remove.", 0.85, 0.85, 0.85, true)
        _G.GameTooltip:AddLine("Right: aggregated materials with owned/missing and prices.", 0.85, 0.85, 0.85, true)
        _G.GameTooltip:Show()
    end)
    ...
    local closeBtn = CreateFrame("Button", nil, planFrame, "BackdropTemplate")
    ...
    closeBtn:SetBackdropColor(bgT[1], bgT[2], bgT[3], bgT[4])
    closeBtn:SetBackdropBorderColor(border[1], border[2], border[3], 0.8)
    local xText = closeBtn:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    xText:SetPoint("CENTER", 0, 1)
    xText:SetText("X")
    xText:SetTextColor(textPrimary[1], textPrimary[2], textPrimary[3], 1)
    closeBtn:SetScript("OnClick", function() planFrame:Hide() end)

    local clearBtn = CreateThemedButton(planFrame, "Clear Craft List", 120)
    clearBtn:SetPoint("TOPRIGHT", closeBtn, "BOTTOMRIGHT", 0, -10)
    ...
    leftPanel:SetBackdropBorderColor(border[1], border[2], border[3], 0.5)
	    ...
	    leftTitle:SetText("Crafting Targets")
        ...
        selectedDesc:SetText("Select an item in the list to see details.")
    ...
    rightTitle:SetText("Aggregated Materials (Owned vs Missing)")
...
function PlanUI:Refresh()
    ...
    if planFrame and planFrame._hv and planFrame._hv.summary then
        planFrame._hv.summary:SetText("Targets: " .. tostring(#itemIDs) .. "  |  Mats (req/miss): 0/0  |  Est. cost (missing): 0")
    end
    if planScrollFrame and planListContainer then
        planListContainer:SetWidth(math.max(1, (planScrollFrame:GetWidth() or 1)))
    end
    ...
    for i = 1, #itemIDs do
        ...
        if known == true then
            btn.subText:SetText(profText .. " (Known)")
        elseif known == false then
        ...
        if not btn._hvTooltipSetup then
            ...
            btn:SetScript("OnEnter", function(selfBtn)
                ...
                AddRecipeKnownToTooltip(GetPlanItemRecipeKnown(selfBtn._hvItemID))
            end)
            btn:SetScript("OnLeave", function(selfBtn)
    ...
    if planFrame and planFrame._hv and planFrame._hv.selectedDesc and planFrame._hv.selectedName and planFrame._hv.selectedIcon then
        ...
        if selectedPlanItemID then
            ...
            planFrame._hv.selectedDesc:SetText("Select an item in the list to see details.")
    ...
    local headerText = "Aggregated Materials (Owned vs Missing)"
    if totals and (totals.missingTotal or 0) > 0 then
        headerText = string.format("Aggregated Materials (Missing %d)", totals.missingTotal or 0)
    end
    ...
    for i = 1, #mats do
        ...
        if unitPrice and unitPrice > 0 then
            ...
            if isVendor and priceLabel == "Vendor" then
                ...
                if btn.totalPriceText then btn.totalPriceText:SetText(FormatMoneyFromCopper(totalCost)) end
                    ...
                    if btn.unitPriceText then btn.unitPriceText:SetText("No price") end
        ...
        if btn.reqLine then
            ...
            btn.reqLine:SetText(string.format("Req %d Own %d Miss %d", required, owned, missing))
        end
	        if btn.invLine then
	            btn.invLine:Hide()
	        end
    ...
    if planFrame and planFrame._hv and planFrame._hv.summary then
        ...
        planFrame._hv.summary:SetText(
            "Targets: " .. tostring(#itemIDs)
            .. "  |  Mats (req/miss): " .. tostring(reqTotal) .. "/" .. tostring(missTotal)
            .. "  |  Miss (alts): " .. tostring(missAlts)
            .. "  |  Est. cost (missing): " .. FormatMoneyFromCopper(estMissing)
        )
    end
end
