...
function MaterialsTrackerUI:CreateFrame()
    ...
    local frame = CreateFrame("Frame", "HousingMaterialsTrackerFrame", UIParent, "BackdropTemplate")
    frame:SetSize(520, 520)
    do
        local pos = GetPositionStore()
    ...
    table.insert(_G.UISpecialFrames, "HousingMaterialsTrackerFrame")
    ...
    if HousingTheme then
        ...
        header:SetBackdropColor(bgSecondary[1], bgSecondary[2], bgSecondary[3], 0.85)
    end

    local headerGradient = header:CreateTexture(nil, "BACKGROUND", nil, 1)
    headerGradient:SetAllPoints()
    headerGradient:SetTexture("Interface\\Buttons\\WHITE8x8")
    local topColor = colors.accentPrimary or {0.55, 0.45, 0.85, 1}
    local bottomColor = colors.bgSecondary or {0.12, 0.12, 0.12, 0.9}
    headerGradient:SetGradient("VERTICAL",
        CreateColor(topColor[1], topColor[2], topColor[3], math.min(1, topColor[4] + 0.1)),
        CreateColor(bottomColor[1], bottomColor[2], bottomColor[3], math.max(0.8, bottomColor[4]))
    )

    local title = header:CreateFontString(nil, "OVERLAY", (HousingTheme and HousingTheme.Fonts.title) or "GameFontNormalLarge")
    title:SetPoint("LEFT", 16, 0)
    title:SetText("Materials Tracker")
    if HousingTheme then
        title:SetTextColor(unpack(HousingTheme.Colors.textPrimary))
    else
    ...
    if HousingTheme then
        ...
        closeBtn:SetBackdropColor(bgPrimary[1], bgPrimary[2], bgPrimary[3], 0.7)
    else
    ...
    local closeText = closeBtn:CreateFontString(nil, "OVERLAY", (HousingTheme and HousingTheme.Fonts.normal) or "GameFontNormal")
    closeText:SetPoint("CENTER")
    closeText:SetText(L and L["BUTTON_CLOSE_X"] or "X")
    if HousingTheme then
        closeText:SetTextColor(unpack(HousingTheme.Colors.textSecondary))
    else
    ...
    closeBtn:SetScript("OnEnter", function(self)
        if HousingTheme then
            ...
            closeText:SetTextColor(unpack(HousingTheme.Colors.textHighlight))
        else
