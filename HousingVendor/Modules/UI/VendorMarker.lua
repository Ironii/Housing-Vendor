...
local function CalculateDistance(vendorCoords)
    ...
    if HousingWaypointManager and HousingWaypointManager.GetActiveWaypointDistance and HousingWaypointManager.GetActiveWaypointInfo then
        ...
        if info and (info.npcID and currentNPCID and info.npcID == currentNPCID or (info.name and currentVendorName and info.name == currentVendorName)) then
...
function VendorMarker:MarkVendor(color, colorName, raidIcon, texture)
    ...
    print("|cFF8A7FD4HousingVendor:|r Marked " .. (currentVendorName or "vendor") .. " with Blue Moon (icon + colored name)")
...
function VendorMarker:ClearVendorMarker()
    ...
    print("|cFF8A7FD4HousingVendor:|r Cleared marker from " .. (currentVendorName or "vendor"))
...
local function ApplyNameplateHighlight(nameplate, npcID, enableDebug, hasRaidIcon)
    ...
    -- Apply or restore color and raid icon
    if markerData and markerData.color and nameText then
        ...
        -- Add custom icon above nameplate only if a raid icon isn't already shown
        if markerData.texture and not hasRaidIcon then
...
-- Apply raid marker to unit using Blizzard API (works with all nameplate addons)
local function ApplyRaidMarkerToUnit(unit, raidIcon)
    if not unit or not UnitExists(unit) then return false end

    -- Clear existing raid marker first (triggers full event chain)
    SetRaidTarget(unit, 0)

    -- Set new raid marker (this triggers UNIT_RAIDTARGET_CHANGED that Plater/addons listen to)
    if raidIcon and raidIcon > 0 then
        SetRaidTarget(unit, raidIcon)
    end
...
local function OnNameplateAdded(unit)
    ...
    if unitType == "Creature" and npcID then
        ...
        local markerData = storage[npcID]
        if markerData then
            -- Apply raid marker (icon above nameplate)
            if markerData.raidIcon then
                ApplyRaidMarkerToUnit(unit, markerData.raidIcon)
            end

            -- Apply name color highlighting
            local currentMarker = GetRaidTargetIndex(unit)
            local hasRaidIcon = markerData.raidIcon and currentMarker == markerData.raidIcon
            ApplyNameplateHighlight(nameplate, npcID, nil, hasRaidIcon)
        end
    end
end
...
function VendorMarker:RefreshNameplates()
    for unit, npcID in pairs(nameplateFrames) do
        ...
        if markerData then
            ...
            -- Clear raid marker if no marker data
            ApplyRaidMarkerToUnit(unit, 0)

            -- Restore original nameplate colors
            if nameplate then
                ApplyNameplateHighlight(nameplate, npcID, nil, false)
            end
        end
...
function VendorMarker:DebugCurrentNameplate()
    ...
    for unit, npcID in pairs(nameplateFrames) do
        ...
        -- Show current raid marker status
        local currentMarker = GetRaidTargetIndex(unit)
        print("Current raid marker: " .. tostring(currentMarker or "none"))
